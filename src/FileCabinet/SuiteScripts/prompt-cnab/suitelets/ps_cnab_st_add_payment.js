  ...
  function (serverWidget, search, record, redirect, runtime, task, format, paymentBatchDAO) {
    ...
    function onRequest (context) {
      ...
      if (request.method === 'GET') {
        ...
        const paymentTask = task.create({
          ...
          scriptId: 'customscript_ps_cnab_mr_add_payment',
          deploymentId: 'customdeploy_ps_cnab_mr_add_payment',
          params: {
            custscript_ps_payment_batch_to_add: paymentBatchId
          }
        })
    ...
    function _buildInstallmentsList (parameters) {
      ...
      const searchFilters = [{
        ...
        name: 'custbody_ps_cnab_forecast_bank_account',
        operator: search.Operator.NONEOF,
        values: '@NONE@'
      }, {
        name: 'custbody_ps_cnab_payment_type',
        operator: search.Operator.NONEOF,
        values: '@NONE@'
      }, {
        name: 'custbody_ps_cnab_payment_form',
        operator: search.Operator.NONEOF,
        values: '@NONE@'
      }]
        ...
        if (filterSubsidiaryFieldValue) {
          ...
          if (filterForecastBankAccountFieldValue) {
            searchFilters.push({
              name: 'custbody_ps_cnab_forecast_bank_account',
              operator: search.Operator.ANYOF,
              values: filterForecastBankAccountFieldValue
            })
          }
        }
      ...
      search.create({
        ...
        columns: [
          ...
          { name: 'installmentnumber', join: 'installment' },
          { name: 'duedate', join: 'installment' },
          { name: 'amount', join: 'installment' },
      ...
      form.clientScriptModulePath = '../clients/ps_cnab_cl_add_payment.js'
